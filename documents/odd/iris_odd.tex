...
By implementing IRIS using an Entity Component System (ECS) framework, the state variables of the model are organised in so-called ECS \textit{components}, which are attached to the ECS grid cell entities. Table \ref{tab:state_variables} provides an overview of the state variables of the IRIS model.
...
\midrule
\footnotesize{TimeStep} 	& \footnotesize{current} 			& int & $\in \{1,...,365\}$ & \footnotesize{Current time step} \\
\footnotesize{Randomness} 	& \footnotesize{rng} 				& \footnotesize{RandomGenerator} & - & \footnotesize{Pseudo Random Number Generator} \\
\multirow{3}{*}{\footnotesize{Position}} & \footnotesize{x}  	& int  & $\in \{1,...,12\}$  & \footnotesize{x coordinate} \\
							& \footnotesize{y}    				& int  & $\in \{1,...,12\}$ & \footnotesize{y coordinate} \\
...
\midrule
\multirow{11}{*}{\footnotesize{TickAbundance}} & \footnotesize{LARVAE}		& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of questing larvae} \\
							& \footnotesize{NYMPH}				& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of questing nymphs} \\
							& \footnotesize{ADULT}				& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of questing adults} \\
							& \footnotesize{LARVAE\_INACTIVE}	& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of inactive larvae} \\
							& \footnotesize{NYMPH\_INACTIVE}	& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of inactive nymphs} \\
							& \footnotesize{ADULT\_INACTIVE}	& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of inactive adults} \\
							& \footnotesize{LARVAE\_ENGORGED}	& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of engorged larvae} \\
							& \footnotesize{NYMPH\_ENGORGED}	& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of engorged nymphs} \\
							& \footnotesize{ADULT\_ENGORGED}	& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of engorged adults} \\
							& \tiny{LARVAE\_LATE\_ENGORGED}		& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of late engorged larvae} \\
							& \tiny{NYMPHS\_LATE\_ENGORGED}		& \small{LifeCycleStage} & $\in \mathbb{N}$ & \footnotesize{No. of late engorged nymphs} \\
\midrule
\multirow{3}{*}{\footnotesize{Temperature}} & \small{meanTemperature}  & double  	& $\in \mathbb{R}$ & \footnotesize{Mean temperature} \\
							& \footnotesize{minTemperature}   & double  	& $\in \mathbb{R}$ & \footnotesize{Minimum temperature} \\
							& \footnotesize{maxTemperature}   & double  	& $\in \mathbb{R}$ & \footnotesize{Maximum temperature} \\
\footnotesize{Humidity} 	& \footnotesize{relativeHumidity} & double  	& $\in \{0,...,100\}$ & \footnotesize{Relative humidity} \\
\bottomrule
\end{tabularx}
\end{table}
