...
\textbf{Stochasticity:} To determine random numbers we use the seedable random number generator \emph{Mersenne-Twister} provided by the Apache Commons Math library (version 3.6.1.) and implemented a method \inlinecode{roundRandom()} to allow processes that statistically affect less than a single tick to still happen. State transitions between tick cohorts are based on rates, but integer numbers of individuals are stored into the cohorts. To break down the probability rates to this individual level, the rounding is therefore stochastic. Otherwise, the number of ticks in cohorts could never become zero in some cases when normal rounding was applied. The random rounding algorithm is presented below.
...
Also, the translocation process of the submodel \emph{Feeding} described in section~\ref{feeding} uses stochastisity through distance-dependent probabilities. The translocation to a distant grid cell occurs with a lower probability than the translocation to a nearby grid cell. 
